...
```mermaid
graph TD
    subgraph WD_SUB["Consumer Wearable"]
        WD[Apple Watch] -- "Records Single-Lead ECG (Lead I)" --> DB[(Apple Health DB)]
    end

    subgraph IP_SUB["iPhone Companion App (SwiftUI & Swift Charts)"]
    ...
    subgraph PY_SUB["Python Processing Engine (FastAPI)"]
    ...
    subgraph OUT_SUB["Clinical Output (Generated on iPhone)"]
